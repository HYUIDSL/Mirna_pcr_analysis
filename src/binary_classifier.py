...
class BinaryClassifier:
    ...
    def cv_logistic_regression(self):
        ...
        for train_index, val_index in self.kf.split(self.X, self.y_binary):
    ...
    def cv_bayesian_logistic_regression(self):
        ...
        for train_index, val_index in self.kf.split(self.X, self.y_binary):
            ...
            with pm.Model() as weighted_logistic_model:
                ...
                logit_p = alpha + pm.math.dot(X_train_scaled, beta)
            ...
            all_probs = [1 / (1 + np.exp(-(alpha_s + np.dot(X_val_scaled, beta_s)))) for alpha_s, beta_s in zip(alpha_samples, beta_samples)]
    ...
    def fit_final_models(self, draws=500, tune=500):
        ...
        self.final_logistic_model.fit(X_scaled, self.y_binary, sample_weight=self.weights)
